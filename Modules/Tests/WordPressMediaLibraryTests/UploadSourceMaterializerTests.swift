...
struct UploadSourceMaterializerTests {
    ...
    func heicToJPEGPreservesOrientation() throws {
        ...
        let outData = try Data(contentsOf: result.tempFileURL)
        let src = try #require(CGImageSourceCreateWithData(outData as CFData, nil))
    ...
    func imageWithinMaxDimensionPassesThrough() async throws {
        ...
        try smallJPEG.write(to: sourceURL)
        defer { try? FileManager.default.removeItem(at: sourceURL) }
        ...
        let m = UploadSourceMaterializer(policy: capPolicy, temporaryRoot: inspectableRoot)
        let result = try await m.materialize(source: .file(sourceURL), into: stage())

        // Byte-for-byte equality: any ImageIO re-encode would alter the bytes.
        let outBytes = try Data(contentsOf: result.tempFileURL)
        #expect(outBytes == smallJPEG)
    }
...
/// Synthetic JPEG carrying a GPS dictionary, for location-stripping tests.
private func makeJPEGWithGPS() throws -> Data {
    let image = makeSolidColorImage(size: CGSize(width: 64, height: 64), color: .green)
    let gps: [CFString: Any] = [
        kCGImagePropertyGPSLatitude: 37.33,
        kCGImagePropertyGPSLongitude: -122.03
    ]
    return try encodeImage(image, as: .jpeg, properties: [kCGImagePropertyGPSDictionary: gps])
}
