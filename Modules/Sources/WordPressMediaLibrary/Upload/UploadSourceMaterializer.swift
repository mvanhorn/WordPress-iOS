...
final class UploadSourceMaterializer: MediaSourceMaterializing, Sendable {
    ...
    private func heicToJPEG(data: Data) throws -> Data {
        ...
        // Copy the image straight from source to destination. ImageIO carries
        // the orientation tag (and other metadata) across the container change,
        // so the stored pixels stay paired with their orientation. Decoding to a
        // bare CGImage and re-adding it would drop the tag and render a
        // non-upright source (e.g. a 180°-oriented HEIC) upside down.
        CGImageDestinationAddImageFromSource(
            dst,
            src,
            0,
            [kCGImageDestinationLossyCompressionQuality: policy.imageJpegQuality] as CFDictionary
        )
    ...
    private func resizeIfNeeded(data: Data, contentType: UTType) throws -> Data {
        ...
        // The thumbnail generator never upscales, so an image already within the
        // limit would only be re-encoded (lossily) at its current size. Reading
        // the stored pixel dimensions is a cheap header parse, and max(w, h) is
        // orientation-independent, so the check is correct regardless of the
